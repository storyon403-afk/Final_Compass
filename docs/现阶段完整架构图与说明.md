...
路由表（src/main.js:16-28，createWebHistory）：
...
- 旧路径 /ai-analysis、/ai-center/workflow 均为重定向（src/main.js:19,22）；AuthView 与 ModuleMaintenanceView 不入路由，由 App.vue 按认证状态/模块状态条件渲染。
...
- 无 Pinia/Vuex：会话与身份用 src/api.js 模块级 ref/computed 单例（authSession、authenticated、isAdmin、profile，api.js:9-12）；AI 凭据面板状态在 src/aiCenterSettings.js 的 reactive 单例。
- 统一 request()/requestBlob() 封装：自动附加 Authorization: Bearer <token>（api.js:16,33），401 时清会话，错误透传 X-Trace-Id。
- 会话令牌双通道：主通道 localStorage + Bearer 头；CET 音频等媒体标签无法带头，先 POST /api/auth/stream-cookie 换取 HttpOnly Cookie（api.js:329，AuthController.java:50-57），音频再以 Cookie 鉴权加载。
...
- **认证模型**：无 Spring Security 全家桶（仅 spring-security-crypto 的 BCrypt）。登录后 DB 表 login_session 存随机 UUID 令牌（7 天过期）；令牌经 Bearer 头或 finals_compass_session Cookie 提取（AuthenticationInterceptor.java:33-42），查 login_session JOIN app_user 校验（AuthService.java:58-79）。
...
1. **登录**：前端 → POST /api/auth/login（BCrypt 比对）→ 写 login_session（7 天）→ 返回令牌（同时设 Cookie）→ localStorage 持久化 → 后续请求 Bearer 携带。
