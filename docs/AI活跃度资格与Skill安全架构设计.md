...
> Key 将采用 AES-256-GCM 加密保存；管理员界面无法查看原文。服务器仅在代表当前用户发起模型请求时短暂解密，用户可以随时删除已保存的 Key。
...
新增 Skill 时实现 `AiSkill` 并注册为 Spring Bean；`AiSkillRegistry` 会自动暴露元数据，并且只允许调用已注册 ID。
