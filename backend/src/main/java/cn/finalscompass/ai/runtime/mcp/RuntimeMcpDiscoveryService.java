...
public final class RuntimeMcpDiscoveryService {
    ...
    public RuntimeMcpDiscoveryService(RuntimeMcpServerRepository servers,
                                      RuntimeMcpTransportRegistry transports,
                                      RuntimeMcpCredentialResolverRegistry credentials,
                                      RuntimeMcpDiscoveryStore snapshots, ObjectMapper json) {
    ...
    public RuntimeMcpDiscoveryReport discover(String serverKey, long requestedByUserId) {
        ...
        RuntimeMcpServerDefinition server = servers.findActiveByKey(serverKey)
        ...
        return new RuntimeMcpDiscoveryReport(snapshot.discoveryId(), persisted.snapshotId(), server.serverKey(),
                snapshot.schemaDigest(), snapshot.tools().size(), persisted.staleBindings());
    }

    private RuntimeMcpDiscoverySnapshot normalize(RuntimeMcpServerDefinition server,
                                                   RuntimeMcpDiscoveryResult result) {
        if (result == null || !server.protocolVersion().equals(result.protocolVersion())
        ...
        for (RuntimeMcpDiscoveredTool source : result.tools()) {
            if (source.name() == null || !source.name().matches("[A-Za-z0-9][A-Za-z0-9_.:/-]{0,159}")
                    || !names.add(source.name()))
                throw new IllegalArgumentException("Runtime MCP discovered Tool name is invalid or duplicated");
            String input = canonicalObject(source.inputSchemaJson(), "MCP input schema");
            String output = source.outputSchemaJson() == null ? null
            ...
            tools.add(new RuntimeMcpNormalizedTool(source.name(), limited(source.title(), 200),
                    limited(source.description(), 2000, ""), input, output, annotations, digest));
        }
        tools.sort(Comparator.comparing(RuntimeMcpNormalizedTool::name));
        StringBuilder manifest = new StringBuilder(result.protocolVersion()).append('\n').append(capabilities);
        tools.forEach(tool -> manifest.append('\n').append(tool.name()).append(':').append(tool.schemaDigest()));
        return new RuntimeMcpDiscoverySnapshot(UUID.randomUUID().toString(), server.id(),
                result.protocolVersion(), capabilities, digest(manifest.toString()), tools);
    }
    ...
    private String limited(String value, int max) { return limited(value, max, null); }
