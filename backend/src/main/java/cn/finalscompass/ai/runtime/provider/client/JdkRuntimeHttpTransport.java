...
public final class JdkRuntimeHttpTransport implements RuntimeHttpTransport {
    ...
    public RuntimeHttpResponse postJson(RuntimeHttpRequest request) {
        ...
        try {
            ...
            return new RuntimeHttpResponse(response.statusCode(), response.headers().map(),
                    new String(body, StandardCharsets.UTF_8));
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Runtime Provider request interrupted", exception);
        } catch (RuntimeException exception) { throw exception; }
        catch (Exception exception) { throw new IllegalStateException("Runtime Provider request failed", exception); }
    }

    private void validate(RuntimeHttpRequest request) {
        if (request == null || request.uri() == null || !allowedUri(request.uri())
                || request.connectTimeout() == null
                || request.requestTimeout() == null || request.connectTimeout().isNegative()
                || request.connectTimeout().isZero() || request.requestTimeout().isNegative()
                || request.requestTimeout().isZero() || request.requestTimeout().compareTo(request.connectTimeout()) < 0
                || request.body() == null || request.maximumResponseBytes() < 1024
