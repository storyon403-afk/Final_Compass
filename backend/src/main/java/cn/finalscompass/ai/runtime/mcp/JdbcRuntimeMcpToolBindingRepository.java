...
public class JdbcRuntimeMcpToolBindingRepository implements RuntimeMcpToolBindingRepository {
    ...
    public JdbcRuntimeMcpToolBindingRepository(JdbcClient jdbc) { this.jdbc = jdbc; }

    @Override public Optional<RuntimeMcpToolBinding> findActive(String toolKey, String toolVersion) {
        ...
        return jdbc.sql("""
                ...
                """).param("toolKey", toolKey).param("toolVersion", toolVersion)
                .query((result, row) -> {
                    ...
                    RuntimeMcpServerDefinition server = new RuntimeMcpServerDefinition(
                            result.getLong("id"), result.getString("server_key"), result.getString("name"),
                            transport, endpoint, result.getString("protocol_version"),
                    ...
                    return new RuntimeMcpToolBinding(result.getString("tool_key"), result.getString("version"),
                            server, result.getString("remote_tool_name"),
    ...
    private void validateEndpoint(RuntimeMcpTransportType transport, String value) {
        if (transport == RuntimeMcpTransportType.STDIO) {
            if (value != null) throw new IllegalStateException("STDIO MCP Server cannot have endpoint URI");
            return;
        }
