...
@Component
public final class GeminiGenerateContentRuntimeProviderClient implements RuntimeProviderProtocolClient {
    private static final String ADAPTER_KEY = "gemini-generate-content-v1";
    private final ObjectMapper json;
    private final RuntimeHttpTransport transport;

    public GeminiGenerateContentRuntimeProviderClient(ObjectMapper json, RuntimeHttpTransport transport) {
        this.json = json;
    ...
    public RuntimeProviderClientResult invoke(RuntimeModelInvocationCommand command,
        ...
        try {
            ...
            RuntimeHttpResponse response = transport.postJson(new RuntimeHttpRequest(
                    ...
                    Duration.ofMillis(command.connectTimeoutMs()), Duration.ofMillis(command.timeoutMs()),
                    Map.of("x-goog-api-key", new String(credential.apiKey()),
                            "Content-Type", "application/json"), body, 8 * 1024 * 1024));
            if (response.statusCode() / 100 != 2)
                throw failure("GEMINI_HTTP_" + response.statusCode(), response.statusCode(),
            ...
            return new RuntimeProviderClientResult(content(parsed, toolCalls), usage.path("promptTokenCount").asInt(0),
                    usage.path("candidatesTokenCount").asInt(0), false,
                    requestId(response.headers()), toolCalls,
    ...
    public RuntimeProviderClientResult continueWithToolResults(
        ...
        try {
            ...
            var contents = ((com.fasterxml.jackson.databind.node.ArrayNode) state.path("contents")).deepCopy();
            List<Map<String, Object>> responses = new ArrayList<>();
            Map<String, String> providerNames = command.toolSpecifications().stream().collect(
                    java.util.stream.Collectors.toMap(RuntimeToolSpecification::toolKey,
                            RuntimeToolSpecification::providerName));
            ...
            return new RuntimeProviderClientResult(content(parsed, calls), usage.path("promptTokenCount").asInt(0),
                    usage.path("candidatesTokenCount").asInt(0), false, requestId(response.headers()), calls,
    ...
    private Map<String, Object> requestBody(RuntimeModelInvocationCommand command,
        ...
        parts.add(Map.of("text", command.userInput() + "\n\n[Runtime Context]\n" + command.contextJson()));
        if (binaryInput != null) {
            byte[] bytes = binaryInput.copyBytes();
            try {
                parts.add(Map.of("inlineData", Map.of("mimeType", binaryInput.mediaType(),
        ...
        body.put("systemInstruction", Map.of("parts", List.of(Map.of("text", command.systemInstruction()))));
        body.put("contents", List.of(Map.of("role", "user", "parts", parts)));
        body.put("generationConfig", generationConfig);
        if (!command.toolSpecifications().isEmpty()) body.put("tools", List.of(Map.of(
                "functionDeclarations", command.toolSpecifications().stream().map(tool -> Map.of(
                        "name", tool.providerName(), "description", tool.description(),
                        "parameters", schema(tool.inputSchemaJson()))).toList())));
        return body;
    }

    private Map<String, Object> continuationBody(RuntimeModelInvocationCommand command, JsonNode contents) {
        Map<String, Object> generationConfig = new LinkedHashMap<>();
        generationConfig.put("temperature", 0.2);
        ...
        body.put("generationConfig", generationConfig);
        body.put("tools", List.of(Map.of("functionDeclarations", command.toolSpecifications().stream()
                .map(tool -> Map.of("name", tool.providerName(), "description", tool.description(),
                        "parameters", schema(tool.inputSchemaJson()))).toList())));
        return body;
    }
    ...
    private void validate(RuntimeModelInvocationCommand command, ResolvedAiCredential credential,
        ...
        if (command == null || credential == null || !ADAPTER_KEY.equals(command.adapterKey())
                || !"gemini".equals(command.providerKey()) || command.connectTimeoutMs() < 100
    ...
    private void validateTools(RuntimeModelInvocationCommand command) {
        if (command.allowedTools().size() != command.toolSpecifications().size()
            ...
            throw new IllegalArgumentException("Gemini Runtime Tool specifications do not match allowlist");
    }
    ...
    private URI endpoint(String value, String modelKey) {
        ...
        return URI.create(base.toString().replaceAll("/+$", "")
    ...
    private List<RuntimeToolCall> toolCalls(JsonNode parsed, List<RuntimeToolSpecification> specifications) {
        ...
        for (JsonNode candidate : parsed.path("candidates"))
            for (JsonNode part : candidate.path("content").path("parts")) if (part.path("functionCall").isObject()) {
                ...
                String responseId = parsed.path("responseId").asText("response").replaceAll("[^A-Za-z0-9_-]", "_");
                result.add(new RuntimeToolCall("gemini_" + responseId + "_" + index++, toolKey,
    ...
    private String continuationState(JsonNode request, JsonNode response) {
        ...
        catch (Exception exception) { throw failure("GEMINI_CONTINUATION_STATE_INVALID", null, false, exception); }
    }
