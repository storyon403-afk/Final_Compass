...
public final class DeepSeekRuntimeProviderClient implements RuntimeProviderProtocolClient {
    ...
    public RuntimeProviderClientResult invoke(RuntimeModelInvocationCommand command,
        ...
        try {
            ...
            String body = json.writeValueAsString(Map.of(
                    "model", command.modelKey(), "stream", false, "temperature", 0.2,
                    "messages", List.of(
            ...
            if (response.statusCode() / 100 != 2)
                throw failure("DEEPSEEK_HTTP_" + response.statusCode(), response.statusCode(),
            ...
            return new RuntimeProviderClientResult(content, number(usage.get("prompt_tokens")),
                    number(usage.get("completion_tokens")), false, requestId(response.headers()), List.of(), null);
    ...
    private void validate(RuntimeModelInvocationCommand command, ResolvedAiCredential credential,
        ...
        if (command == null || credential == null || !ADAPTER_KEY.equals(command.adapterKey())
                || !"deepseek".equals(command.providerKey()) || command.connectTimeoutMs() < 100
    ...
    private String content(Map<String, Object> parsed) {
        if (!(parsed.get("choices") instanceof List<?> choices) || choices.isEmpty()
                || !(choices.getFirst() instanceof Map<?, ?> choice)
                || !(choice.get("message") instanceof Map<?, ?> message)
                || !(message.get("content") instanceof String value) || value.isBlank())
            throw failure("DEEPSEEK_EMPTY", null, false, null);
        return value;
    }
