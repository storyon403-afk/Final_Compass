...
public class JdbcRuntimeExecutionTraceStore implements RuntimeExecutionTraceStore {
    ...
    private static final java.util.Set<String> FORBIDDEN_JSON_FIELDS = java.util.Set.of(
            "rawinput", "rawoutput", "inputtext", "outputtext", "prompt", "response",
    ...
    public JdbcRuntimeExecutionTraceStore(JdbcClient jdbc, TransactionTemplate transactions,
                                          ObjectMapper json, RuntimeTraceStateMachine states) {
        this.jdbc = jdbc;
    ...
    public long createExecution(CreateRuntimeExecution command) {
        ...
        Long id = transactions.execute(status -> {
            jdbc.sql("""
                    ...
                    """).param("externalId", command.executionId()).param("traceId", command.traceId())
                    .param("parentId", command.parentExecutionId()).param("legacyTaskId", command.legacyTaskId())
                    .param("userId", command.userId()).param("sessionId", command.sessionId())
                    .param("runtimeType", command.runtimeType().name()).param("goalSummary", command.goalSummary())
                    .param("inputReference", command.inputReference()).param("workflowKey", command.workflowKey())
                    .param("workflowVersion", command.workflowVersion()).param("metadata", command.metadataJson())
                    .update();
            long executionId = jdbc.sql("SELECT id FROM ai_runtime_execution WHERE execution_id=:externalId")
                    .param("externalId", command.executionId()).query(Long.class).single();
            appendInTransaction(executionId, null, "EXECUTION_CREATED", objectPayload(
                    "status", RuntimeExecutionStatus.CREATED.name(), "runtimeType", command.runtimeType().name()));
            return executionId;
        });
    ...
    public long createNode(CreateRuntimeExecutionNode command) {
        ...
        Long id = transactions.execute(status -> {
            jdbc.sql("""
                    ...
                    """).param("executionId", command.executionId()).param("parentNodeId", command.parentNodeId())
                    .param("nodeKey", command.nodeKey()).param("nodeType", command.nodeType().name())
                    .param("skillId", command.skillId()).param("skillVersionId", command.skillVersionId())
                    .param("skillKey", command.skillKeySnapshot()).param("skillVersion", command.skillVersionSnapshot())
                    .param("attempt", command.attempt()).param("inputReference", command.inputReference())
                    .param("inputDigest", command.inputDigest()).param("metadata", command.metadataJson()).update();
            long nodeId = jdbc.sql("""
                    ...
                    """).param("executionId", command.executionId()).param("nodeKey", command.nodeKey())
                    .param("attempt", command.attempt()).query(Long.class).single();
            appendInTransaction(command.executionId(), nodeId, "NODE_CREATED",
                    objectPayload("nodeKey", command.nodeKey(), "nodeType", command.nodeType().name()));
            return nodeId;
        });
    ...
    public long createProviderInvocation(CreateRuntimeProviderInvocation command) {
        validate(command);
        Long id = transactions.execute(status -> {
            long executionId = jdbc.sql("""
                    SELECT execution_id FROM ai_runtime_execution_node WHERE id=:nodeId FOR UPDATE
                    """).param("nodeId", command.executionNodeId()).query(Long.class).optional()
            ...
            jdbc.sql("""
                    ...
                    """).param("invocationId", command.invocationId()).param("nodeId", command.executionNodeId())
                    .param("providerId", command.providerId()).param("modelId", command.providerModelId())
                    .param("providerKey", command.providerKeySnapshot()).param("modelKey", command.modelKeySnapshot())
                    .param("credentialSource", command.credentialSource().name()).param("attempt", command.attempt())
                    .param("fallbackFromId", command.fallbackFromId()).param("metadata", command.metadataJson()).update();
            long invocationId = jdbc.sql("""
                    SELECT id FROM ai_runtime_provider_invocation WHERE invocation_id=:invocationId
                    """).param("invocationId", command.invocationId()).query(Long.class).single();
            appendInTransaction(executionId, command.executionNodeId(), "PROVIDER_INVOCATION_CREATED",
                    objectPayload("provider", command.providerKeySnapshot(), "model", command.modelKeySnapshot()));
            return invocationId;
        });
        if (id == null) throw new IllegalStateException("Provider invocation transaction returned no id");
        return id;
    }

    @Override
    public void transitionExecution(long executionId, RuntimeExecutionStatus target,
                                    String resultReference, String errorCode, String errorSummary) {
        validateTransitionTarget(target, resultReference, null, errorCode, errorSummary);
        transactions.executeWithoutResult(status -> {
            RuntimeExecutionStatus current = jdbc.sql(
                    "SELECT status FROM ai_runtime_execution WHERE id=:id FOR UPDATE")
                    .param("id", executionId).query(String.class).optional()
            ...
            jdbc.sql("""
                    ...
                    .param("terminal", target.terminal()).param("resultReference", resultReference)
                    .param("errorCode", errorCode).param("errorSummary", errorSummary)
                    .param("id", executionId).update();
            appendInTransaction(executionId, null, "EXECUTION_STATUS_CHANGED",
                    objectPayload("from", current.name(), "to", target.name()));
        });
    }

    @Override
    public void transitionNode(long nodeId, RuntimeExecutionNodeStatus target,
                               String outputReference, String outputDigest,
                               String errorCode, String errorSummary) {
        validateTransitionTarget(target, outputReference, outputDigest, errorCode, errorSummary);
        transactions.executeWithoutResult(status -> {
            NodeState row = jdbc.sql("""
                    SELECT execution_id,status FROM ai_runtime_execution_node WHERE id=:id FOR UPDATE
                    """).param("id", nodeId).query(NodeState.class).optional()
                    .orElseThrow(() -> new IllegalArgumentException("Execution node does not exist"));
            ...
            jdbc.sql("""
                    ...
                    WHERE id=:id
                    """).param("target", target.name())
                    .param("startNode", target != RuntimeExecutionNodeStatus.PENDING
                            && target != RuntimeExecutionNodeStatus.READY)
                    .param("terminal", target.terminal()).param("outputReference", outputReference)
                    .param("outputDigest", outputDigest).param("errorCode", errorCode)
                    .param("errorSummary", errorSummary).param("id", nodeId).update();
            appendInTransaction(row.executionId(), nodeId, "NODE_STATUS_CHANGED",
                    objectPayload("from", current.name(), "to", target.name()));
        });
    }

    @Override
    public void transitionProviderInvocation(long invocationId, RuntimeProviderInvocationStatus target,
                                             RuntimeProviderInvocationResult result) {
        RuntimeProviderInvocationResult value = result == null
        ...
        transactions.executeWithoutResult(status -> {
            InvocationState row = jdbc.sql("""
                    ...
                    """).param("id", invocationId).query(InvocationState.class).optional()
                    .orElseThrow(() -> new IllegalArgumentException("Provider invocation does not exist"));
            RuntimeProviderInvocationStatus current = RuntimeProviderInvocationStatus.valueOf(row.status());
            states.requireTransition(current, target);
            jdbc.sql("""
                    UPDATE ai_runtime_provider_invocation SET status=:target,
                      ...
                      completed_at=CASE WHEN :terminal THEN CURRENT_TIMESTAMP(6) ELSE NULL END
                    WHERE id=:id
                    """).param("target", target.name()).param("inputUnits", value.inputUnits())
                    .param("outputUnits", value.outputUnits()).param("estimatedCost", value.estimatedCost())
                    .param("currency", value.currency()).param("latencyMs", value.latencyMs())
                    .param("providerRequestId", value.providerRequestId()).param("errorCode", value.errorCode())
                    .param("errorSummary", value.errorSummary()).param("metadata", value.metadataJson())
                    .param("terminal", target.terminal()).param("id", invocationId).update();
            appendInTransaction(row.executionId(), row.executionNodeId(), "PROVIDER_INVOCATION_STATUS_CHANGED",
                    objectPayload("from", current.name(), "to", target.name()));
        });
    }
    ...
    public long appendEvent(long executionId, Long nodeId, String eventType, String payloadJson) {
        ...
        Long sequence = transactions.execute(
    ...
    private long appendInTransaction(long executionId, Long nodeId, String eventType, String payloadJson) {
        validateEvent(eventType, payloadJson);
        long sequence = jdbc.sql("""
                SELECT next_event_sequence FROM ai_runtime_execution WHERE id=:id FOR UPDATE
                """).param("id", executionId).query(Long.class).optional()
                .orElseThrow(() -> new IllegalArgumentException("Execution does not exist"));
        jdbc.sql("UPDATE ai_runtime_execution SET next_event_sequence=next_event_sequence+1 WHERE id=:id")
                .param("id", executionId).update();
        jdbc.sql("""
                ...
                """).param("executionId", executionId).param("nodeId", nodeId).param("sequence", sequence)
                .param("eventType", eventType).param("payload", payloadJson).update();
        return sequence;
    }

    private void validate(CreateRuntimeExecution command) {
        if (command == null || !externalId(command.executionId(), 64) || !externalId(command.traceId(), 80)
                || command.userId() <= 0 || command.runtimeType() == null)
            throw new IllegalArgumentException("Execution identity is invalid");
        text(command.sessionId(), 80); text(command.goalSummary(), 1000); text(command.inputReference(), 255);
        text(command.workflowKey(), 100); text(command.workflowVersion(), 32);
        object(command.metadataJson(), "metadata");
    }

    private void validate(CreateRuntimeExecutionNode command) {
        if (command == null || command.executionId() <= 0 || command.nodeType() == null
                || command.nodeKey() == null || !NODE_KEY.matcher(command.nodeKey()).matches()
                || command.attempt() <= 0) throw new IllegalArgumentException("Execution node identity is invalid");
        boolean noSkill = command.skillId() == null && command.skillVersionId() == null
                && command.skillKeySnapshot() == null && command.skillVersionSnapshot() == null;
        boolean fullSkill = command.skillId() != null && command.skillVersionId() != null
                && command.skillKeySnapshot() != null && command.skillVersionSnapshot() != null;
        if (!noSkill && !fullSkill) throw new IllegalArgumentException("Execution node skill snapshot is incomplete");
    ...
    private void validate(CreateRuntimeProviderInvocation command) {
        if (command == null || !externalId(command.invocationId(), 64) || command.executionNodeId() <= 0
                || command.providerId() <= 0 || command.providerModelId() <= 0
                || command.providerKeySnapshot() == null
                || !PROVIDER_KEY.matcher(command.providerKeySnapshot()).matches()
                || command.modelKeySnapshot() == null || !MODEL_KEY.matcher(command.modelKeySnapshot()).matches()
                || command.credentialSource() == null || command.attempt() <= 0)
    ...
    private void validate(RuntimeProviderInvocationResult result) {
        if (result.inputUnits() < 0 || result.outputUnits() < 0
        ...
        boolean priced = result.estimatedCost() != null && result.currency() != null
                && result.currency().matches("^[A-Z]{3}$");
        if (!noCost && !priced) throw new IllegalArgumentException("Provider invocation cost is invalid");
        text(result.providerRequestId(), 160);
        object(result.metadataJson(), "metadata");
    }

    private void validateTransitionTarget(Enum<?> target, String reference, String digest,
                                          String errorCode, String errorSummary) {
        ...
        text(reference, 255); text(errorSummary, 500); digest(digest);
    }
    ...
    private void object(String value, String field) {
        ...
        catch (Exception exception) { throw new IllegalArgumentException(field + " is invalid"); }
    }
    private void rejectSensitiveFields(JsonNode node, String field) {
        if (node.isObject()) node.properties().forEach(entry -> {
            ...
            if (FORBIDDEN_JSON_FIELDS.contains(normalized))
                throw new IllegalArgumentException(field + " contains forbidden raw or secret data");
            rejectSensitiveFields(entry.getValue(), field);
        });
        else if (node.isArray()) node.forEach(child -> rejectSensitiveFields(child, field));
    }
    private String objectPayload(String firstKey, String firstValue, String secondKey, String secondValue) {
    ...
    private void text(String value, int max) {
        if (value != null && value.length() > max) throw new IllegalArgumentException("Trace field is too long");
    }
