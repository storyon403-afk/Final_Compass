...
public final class OpenAiRuntimeEmbeddingClient implements RuntimeEmbeddingClient {
  ...
  public OpenAiRuntimeEmbeddingClient(RuntimeHttpTransport http, ObjectMapper json) {
    this.http = http;
    this.json = json;
  }
  ...
  public List<float[]> embed(
    ...
    if (inputs == null
        || inputs.isEmpty()
        || inputs.size() > 64
        || inputs.stream().anyMatch(v -> v == null || v.isBlank() || v.length() > 32000))
      throw new IllegalArgumentException("Embedding batch is invalid");
    if (!candidate.provider().key().equals(credential.provider())
        || !candidate.model().key().equals(credential.model()))
      throw new SecurityException("Embedding credential mismatch");
    try {
      String base = candidate.endpoint().baseUrl().replaceAll("/+$", "");
      var response =
          http.postJson(
              new RuntimeHttpRequest(
                  URI.create(base + "/v1/embeddings"),
                  Duration.ofMillis(candidate.endpoint().connectTimeoutMs()),
                  Duration.ofMillis(candidate.endpoint().requestTimeoutMs()),
                  Map.of(
                      "Authorization",
      ...
      if (response.statusCode() / 100 != 2)
        throw new IllegalStateException("Embedding Provider rejected request");
      ...
      for (var item : root.path("data")) {
        ...
        for (int i = 0; i < values.length; i++) {
          double value = vector.get(i).asDouble(Double.NaN);
          if (!Double.isFinite(value))
            throw new IllegalStateException("Embedding contains invalid value");
