...
public final class RuntimeMcpToolHandler implements RuntimeToolHandler {
    ...
    public RuntimeMcpToolHandler(RuntimeMcpToolBindingRepository bindings,
    ...
    @Override public String executorKey() { return EXECUTOR_KEY; }

    @Override
    public String invoke(RuntimeToolDefinition definition, RuntimeToolExecutionContext context,
        ...
        RuntimeMcpToolBinding binding = bindings.findActive(definition.toolKey(), definition.version())
                .orElseThrow(() -> new IllegalStateException("Runtime MCP Tool binding is unavailable"));
        ...
        try (RuntimeMcpCredential credential = credentials.resolve(server, context.userId())) {
            ...
            try {
                RuntimeMcpCallResult result = transports.require(server.transportType()).callTool(
                        new RuntimeMcpCallRequest(server, binding.remoteToolName(), argumentsJson,
                                context.executionId(), context.nodeId(), context.userId()), token);
