...
public final class RuntimeMcpOAuthService {
    ...
    public RuntimeMcpOAuthService(JdbcClient jdbc, StringRedisTemplate redis, AiSecretCipher cipher,
                                  ObjectMapper json,
                                  @Value("${app.ai.mcp.oauth.redirect-uri:http://127.0.0.1:8080/api/system/mcp/oauth/callback}") String redirectUri,
                                  @Value("${app.environment:dev}") String environment) {
        this.jdbc = jdbc; this.redis = redis; this.cipher = cipher; this.json = json;
        this.redirectUri = redirectUri; this.statePrefix = "fc:" + environment + ":mcp-oauth:";
    }
    ...
    public String authorizationUrl(long adminId, String serverKey) {
        ...
        try {
            redis.opsForValue().set(statePrefix + state, json.writeValueAsString(Map.of(
                    "adminId", adminId, "serverId", config.id(), "serverKey", config.serverKey(),
                    "subject", subject, "verifier", verifier)), Duration.ofMinutes(8));
        } catch (Exception exception) { throw new IllegalStateException("MCP OAuth state could not be saved", exception); }
        return config.authorizationEndpoint() + "?client_id=" + enc(config.clientId())
                + "&response_type=code&redirect_uri=" + enc(redirectUri)
                + "&scope=" + enc(config.scopes()) + "&state=" + enc(state)
                + "&code_challenge=" + enc(challenge) + "&code_challenge_method=S256"
                + "&resource=" + enc(config.endpointUri());
    }
    ...
    public String complete(String code, String state) {
        ...
        try {
            ...
            Map<String, Object> token = token(config, Map.of("grant_type", "authorization_code",
                    "code", code, "redirect_uri", redirectUri,
                    "code_verifier", String.valueOf(values.get("verifier"))));
            save(config, subject, adminId, token);
            return config.serverKey();
        } catch (RuntimeException exception) { throw exception; }
        catch (Exception exception) { throw new IllegalStateException("MCP OAuth callback failed", exception); }
    }
    ...
    public RuntimeMcpCredential resolve(RuntimeMcpServerDefinition server, long userId) {
        ...
        TokenRow row = jdbc.sql("""
                ...
                """).param("serverId", server.id()).param("userId", subject).query(TokenRow.class).optional()
                .orElseThrow(() -> new SecurityException("MCP OAuth connection is unavailable"));
        if (row.expiresAt() != null && row.expiresAt().toInstant().isBefore(Instant.now().plusSeconds(60))) {
            if (row.encryptedRefreshToken() == null) throw new SecurityException("MCP OAuth token expired");
        ...
        try { return new RuntimeMcpCredential(access); }
        finally { Arrays.fill(access, '\0'); }
    }
    ...
    private void disconnect(long serverId, long userId) {
        jdbc.sql("UPDATE ai_runtime_mcp_oauth_connection SET status='REVOKED' WHERE server_id=:server AND user_id=:user")
                .param("server", serverId).param("user", userId).update();
    }
    ...
    private void refresh(long serverId, long subject, TokenRow row) {
        ...
        try {
            OAuthConfig config = config(serverId);
            Map<String, Object> token = token(config, Map.of("grant_type", "refresh_token",
                    "refresh_token", new String(refresh)));
            save(config, subject, row.connectedBy(), token);
        } finally { Arrays.fill(refresh, '\0'); }
    }
    ...
    private void save(OAuthConfig config, long subject, long adminId, Map<String, Object> token) {
        ...
        char[] refresh = token.get("refresh_token") == null ? null
                : String.valueOf(token.get("refresh_token")).toCharArray();
        try {
            var encryptedAccess = cipher.encrypt(access);
            AiSecretCipher.EncryptedSecret encryptedRefresh = refresh == null ? null : cipher.encrypt(refresh);
            ...
            jdbc.sql("""
                    ...
                    """).param("server", config.id()).param("user", subject)
                    .param("access", encryptedAccess.ciphertext()).param("accessIv", encryptedAccess.iv())
                    ...
                    .param("expires", Math.max(60, Math.min(expires, 86400))).param("admin", adminId).update();
        } finally {
            Arrays.fill(access, '\0'); if (refresh != null) Arrays.fill(refresh, '\0');
        }
    }
    ...
    private Map<String, Object> token(OAuthConfig config, Map<String, String> fields) {
        try {
            ...
            form.put("client_id", config.clientId()); form.put("scope", config.scopes());
            form.put("resource", config.endpointUri());
            String body = form.entrySet().stream().map(entry -> enc(entry.getKey()) + "=" + enc(entry.getValue()))
                    .collect(java.util.stream.Collectors.joining("&"));
            HttpResponse<String> response = http.send(HttpRequest.newBuilder(URI.create(config.tokenEndpoint()))
                    .timeout(Duration.ofSeconds(15)).header("Content-Type", "application/x-www-form-urlencoded")
                    .POST(HttpRequest.BodyPublishers.ofString(body)).build(), HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() / 100 != 2) throw new IllegalStateException("MCP OAuth token exchange rejected");
            return json.readValue(response.body(), new TypeReference<>() {});
        } catch (RuntimeException exception) { throw exception; }
        catch (Exception exception) { throw new IllegalStateException("MCP OAuth token exchange failed", exception); }
    }
    ...
    private OAuthConfig config(String key) {
        return jdbc.sql("""
                ...
                """).param("key", key).query(OAuthConfig.class).optional()
    ...
    private OAuthConfig config(long id) {
        return jdbc.sql("""
                ...
                """).param("id", id).query(OAuthConfig.class).single();
    }
    private String required(Map<String, Object> token, String field) {
        Object value = token.get(field);
        if (value == null || String.valueOf(value).isBlank()) throw new IllegalStateException("MCP OAuth token is incomplete");
    ...
    private String sha256Url(String value) { try { return Base64.getUrlEncoder().withoutPadding().encodeToString(MessageDigest.getInstance("SHA-256").digest(value.getBytes(StandardCharsets.US_ASCII))); } catch (Exception e) { throw new IllegalStateException(e); } }
    private String enc(String value) { return URLEncoder.encode(value, StandardCharsets.UTF_8); }

    private record OAuthConfig(long id, String serverKey, RuntimeMcpAuthMode authMode, String endpointUri,
                               String authorizationEndpoint, String tokenEndpoint, String clientId, String scopes) {
        OAuthConfig { validate(authorizationEndpoint); validate(tokenEndpoint); }
        private static void validate(String value) { URI uri=URI.create(value); if(!"https".equalsIgnoreCase(uri.getScheme())||uri.getHost()==null||uri.getUserInfo()!=null||uri.getFragment()!=null) throw new IllegalStateException("MCP OAuth endpoint is invalid"); }
    }
    private record TokenRow(String encryptedAccessToken, String accessTokenIv,
                            String encryptedRefreshToken, String refreshTokenIv,
                            java.sql.Timestamp expiresAt, long connectedBy) {}
}
