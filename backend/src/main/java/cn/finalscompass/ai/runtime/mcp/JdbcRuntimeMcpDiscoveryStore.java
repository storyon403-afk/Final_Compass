...
public class JdbcRuntimeMcpDiscoveryStore implements RuntimeMcpDiscoveryStore {
    ...
    public RuntimeMcpDiscoveryPersistResult saveCurrent(RuntimeMcpDiscoverySnapshot snapshot) {
        ...
        if (result == null) throw new IllegalStateException("MCP discovery transaction returned no result");
        return result;
    }
    ...
    private RuntimeMcpDiscoveryPersistResult save(RuntimeMcpDiscoverySnapshot snapshot) {
        jdbc.sql("""
                ...
                """).param("serverId", snapshot.serverId()).update();
        jdbc.sql("""
                ...
                """).param("serverId", snapshot.serverId()).param("discoveryId", snapshot.discoveryId())
                .param("protocolVersion", snapshot.protocolVersion())
                .param("capabilities", snapshot.capabilitiesJson()).param("toolCount", snapshot.tools().size())
                .param("schemaDigest", snapshot.schemaDigest()).update();
        long snapshotId = jdbc.sql("SELECT LAST_INSERT_ID()").query(Long.class).single();
        for (RuntimeMcpNormalizedTool tool : snapshot.tools()) jdbc.sql("""
                ...
                """).param("snapshotId", snapshotId).param("name", tool.name()).param("title", tool.title())
                .param("description", tool.description()).param("inputSchema", tool.inputSchemaJson())
                .param("outputSchema", tool.outputSchemaJson()).param("annotations", tool.annotationsJson())
                .param("schemaDigest", tool.schemaDigest()).update();
        int stale = jdbc.sql("""
                ...
                """).param("serverId", snapshot.serverId()).param("snapshotId", snapshotId).update();
        jdbc.sql("""
