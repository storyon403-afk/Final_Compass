...
public final class RuntimeMcpAdminController {
  ...
  public RuntimeMcpAdminController(
      AuthService auth,
      RuntimeMcpAdminService admin,
  ...
  @PostMapping("/servers/{key}/discover")
  public Object discover(HttpServletRequest request, @PathVariable String key) {
    return admin.discover(key, auth.requireAdmin(request).id());
  }

  @PutMapping("/servers")
  public void saveServer(
      HttpServletRequest request, @RequestBody RuntimeMcpAdminService.ServerInput input) {
    admin.saveServer(input, auth.requireAdmin(request).id());
  }

  @GetMapping("/servers/{key}/diff")
  public List<Map<String, Object>> diff(HttpServletRequest request, @PathVariable String key) {
    auth.requireAdmin(request);
    return admin.diff(key);
  }

  @PostMapping("/approvals")
  public void requestApproval(HttpServletRequest request, @RequestBody ApprovalRequest input) {
    var user = auth.requireAdmin(request);
    admin.requestApproval(
        input.discoveredToolId(),
        input.toolKey(),
        input.version(),
        input.riskLevel(),
        input.permissions(),
  ...
  public void decide(
      HttpServletRequest request, @PathVariable long id, @RequestBody Decision input) {
    var user = auth.requireAdmin(request);
    admin.decide(id, input.approve(), input.note(), user.id());
  }

  @PostMapping("/servers/{key}/oauth/authorize")
  public Map<String, String> authorize(HttpServletRequest request, @PathVariable String key) {
    return Map.of("authorizationUrl", oauth.authorizationUrl(auth.requireAdmin(request).id(), key));
  }

  @DeleteMapping("/servers/{key}/oauth")
  public void disconnect(HttpServletRequest request, @PathVariable String key) {
  ...
  public ResponseEntity<String> callback(@RequestParam String code, @RequestParam String state) {
    String server = oauth.complete(code, state);
    String origin = frontendOrigin.replace("'", "");
    return ResponseEntity.ok(
        "<!doctype html><meta charset='utf-8'><p>MCP OAuth 已连接，可以关闭窗口。</p>"
