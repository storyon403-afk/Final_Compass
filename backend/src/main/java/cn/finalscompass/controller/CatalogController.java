...
public class CatalogController {
  ...
  public CatalogController(JdbcClient jdbc, AuthService auth) {
    this.jdbc = jdbc;
    this.auth = auth;
  }
  ...
  public College addCollege(
      HttpServletRequest servletRequest, @Valid @RequestBody CreateCollege request) {
    auth.requireAdmin(servletRequest);
    String name = request.name().trim();
    boolean exists =
        jdbc.sql("SELECT COUNT(*) FROM college WHERE name=:name")
                .param("name", name)
                .query(Integer.class)
                .single()
            > 0;
    ...
    long id =
        jdbc.sql("SELECT id FROM college WHERE name=:name")
            .param("name", name)
            .query(Long.class)
            .single();
  ...
  public Course addCourse(
      HttpServletRequest servletRequest, @Valid @RequestBody CreateCourse request) {
    auth.requireAdmin(servletRequest);
    String name = request.name().trim();
    ...
    boolean collegeExists =
        jdbc.sql("SELECT COUNT(*) FROM college WHERE name=:name AND active=TRUE")
                .param("name", college)
                .query(Integer.class)
                .single()
            > 0;
    if (!collegeExists) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "请先添加所属学院");
    CourseIdentity existing =
        jdbc.sql(
                """
            ...
            .param("code", code)
            .param("name", name)
            .query(CourseIdentity.class)
            .optional()
            .orElse(null);
    if (existing == null) {
      CourseIdentity conflict =
          jdbc.sql(
                  """
                  SELECT id,slug,name,category FROM course
      ...
      if (conflict != null) {
        throw new ResponseStatusException(
            HttpStatus.CONFLICT, "课程代码 " + code + " 已属于“" + conflict.name() + "”，请检查课程名称");
      }
    }
    if (existing != null) {
      jdbc.sql(
              ...
              """)
          .param("course", existing.id())
          .param("college", college)
          .param("program", programName)
    ...
    jdbc.sql(
            "INSERT INTO course(slug,name,code,category,college,program_name,course_type) VALUES"
                + " (:slug,:name,:code,:category,:college,:program,:type)")
        .param("slug", slug)
        .param("name", name)
        .param("code", code)
        ...
        .update();
    long id =
        jdbc.sql("SELECT id FROM course WHERE slug=:slug")
            .param("slug", slug)
            .query(Long.class)
            .single();
    jdbc.sql(
            "INSERT INTO course_program(course_id,college,program_name,course_type) VALUES"
                + " (:course,:college,:program,:type)")
        .param("course", id)
        .param("college", college)
        .param("program", programName)
        .param("type", courseType)
...
""")
  ...
  public Teacher addTeacher(
      ...
      @Valid @RequestBody CreateTeacher request) {
    auth.requireAdmin(servletRequest);
    long courseId =
        jdbc.sql("SELECT id FROM course WHERE slug=:slug AND active=TRUE")
    ...
    TeacherIdentity identity =
        jdbc.sql("SELECT id, slug FROM teacher WHERE name=:name AND college=:college LIMIT 1")
            .param("name", name)
            .param("college", college)
            .query(TeacherIdentity.class)
            .optional()
            .orElse(null);
    if (identity == null) {
      ...
      jdbc.sql("INSERT INTO teacher(slug,name,college) VALUES (:slug,:name,:college)")
          ...
          .update();
      long teacherId =
          jdbc.sql("SELECT id FROM teacher WHERE slug=:slug")
              .param("slug", slug)
              .query(Long.class)
              .single();
