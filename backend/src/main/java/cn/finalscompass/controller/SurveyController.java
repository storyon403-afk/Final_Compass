...
public class SurveyController {
    ...
    public SurveyController(JdbcClient jdbc, AuthService auth) { this.jdbc = jdbc; this.auth = auth; }
    ...
    @Transactional
    public Map<String, Object> submit(HttpServletRequest request, @Valid @RequestBody Submission body) {
        ...
        var active = jdbc.sql("SELECT id,prompt FROM survey_question WHERE active=TRUE ORDER BY sort_order,id")
                .query(QuestionRow.class).list();
        ...
        jdbc.sql("INSERT INTO survey_submission(user_id,overall_suggestion) VALUES (:user,:suggestion)")
                .param("user", user.id()).param("suggestion", clean(body.overallSuggestion()))
                .update(holder, "id");
        ...
        for (QuestionRow question : active) {
            ...
            jdbc.sql("""
                ...
                """).param("submission", submissionId).param("question", question.id())
                    .param("snapshot", question.prompt()).param("rating", answer.rating())
                    .param("suggestion", clean(answer.suggestion())).update();
        }
    ...
    public Map<String, Object> adminOverview(HttpServletRequest request) {
        ...
        var questions = jdbc.sql("SELECT id,prompt,sort_order,active FROM survey_question ORDER BY sort_order,id")
                .query().listOfRows();
        var submissions = jdbc.sql("""
    ...
    @Transactional
    public Map<String, String> updateQuestions(HttpServletRequest request, @Valid @RequestBody QuestionUpdate body) {
        auth.requireAdmin(request);
        ...
        for (String prompt : body.questions()) {
            ...
            jdbc.sql("INSERT INTO survey_question(prompt,sort_order,active) VALUES (:prompt,:sort,TRUE)")
                    .param("prompt", value).param("sort", order).update();
            order += 10;
        }
    ...
    public record Answer(long questionId, @Min(1) @Max(5) int rating, @Size(max = 1000) String suggestion) {}
    public record Submission(List<@Valid Answer> answers, @Size(max = 2000) String overallSuggestion) {}
