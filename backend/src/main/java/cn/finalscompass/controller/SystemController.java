...
public class SystemController {
    ...
    public Announcement updateAnnouncement(HttpServletRequest request,
                                             @Valid @RequestBody UpdateAnnouncement update) {
        ...
        jdbc.sql("""
            ...
            WHERE id=1
            """).param("content", update.content().trim()).param("enabled", update.enabled())
    ...
    public Map<String, String> moderate(HttpServletRequest request, @PathVariable String type,
        ...
        String table;
        String status;
        if ("RESOURCE".equals(normalizedType)) { table = "resource"; status = "APPROVE".equals(normalizedDecision) ? "PUBLISHED" : "REJECTED"; }
        else if ("DISCUSSION".equals(normalizedType)) { table = "discussion"; status = "APPROVE".equals(normalizedDecision) ? "VISIBLE" : "REMOVED"; }
        ...
        jdbc.sql("INSERT INTO moderation_audit(item_type,item_id,decision,reviewer_id) VALUES (:type,:id,:decision,:reviewer)")
                .param("type", normalizedType).param("id", id).param("decision", normalizedDecision).param("reviewer", admin.id()).update();
    ...
    public Map<String, String> removeDiscussion(HttpServletRequest request, @PathVariable long id) {
        var admin = auth.requireAdmin(request);
        int changed = jdbc.sql("UPDATE discussion SET status='REMOVED' WHERE id=:id AND status='VISIBLE'").param("id", id).update();
        if (changed != 1) throw new IllegalArgumentException("帖子不存在或已删除");
        jdbc.sql("INSERT INTO moderation_audit(item_type,item_id,decision,reviewer_id) VALUES ('DISCUSSION',:id,'REJECT',:reviewer)")
                .param("id", id).param("reviewer", admin.id()).update();
