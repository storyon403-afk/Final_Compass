...
public final class AiChatController {
    ...
    @PostMapping(value = "/sessions/{sessionKey}/messages", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public SseEmitter message(HttpServletRequest request, @PathVariable String sessionKey,
                              @RequestBody AiChatService.ChatRequest body) {
        AuthService.CurrentUser user = auth.current(request);
        SseEmitter emitter = new SseEmitter(5 * 60 * 1000L);
        Thread.ofVirtual().name("ai-chat-" + sessionKey)
