...
public final class AiRuntimeRouterController {
    ...
    public AiRuntimeRouterController(AuthService auth, AiRuntimeRouterService router) {
        this.auth = auth; this.router = router;
    }

    @GetMapping("/runtimes")
    public AiRuntimeRouterService.Catalog catalog(HttpServletRequest request) {
        auth.current(request); return router.catalog();
    }

    @PostMapping("/route")
    public AiRuntimeRouterService.RouteDecision route(HttpServletRequest request,
                                                       @RequestBody AiRuntimeRouterService.RouteRequest body) {
        auth.current(request); return router.route(body);
    }
}
