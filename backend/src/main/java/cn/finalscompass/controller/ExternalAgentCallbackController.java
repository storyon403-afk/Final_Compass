...
public final class ExternalAgentCallbackController {
  ...
  public ExternalAgentCallbackController(
      AiRuntimeDispatchService dispatch, BrowserGatewayService browser) {
  ...
  public Map<String, Object> status(
      HttpServletRequest request, @PathVariable String runKey, @RequestBody StatusReport body) {
    long runId = authenticate(request, runKey);
  ...
  public Map<String, Object> artifact(
      HttpServletRequest request, @PathVariable String runKey, @RequestBody ArtifactUpload body) {
    long runId = authenticate(request, runKey);
    if (body == null
        || body.fileName() == null
        || body.fileName().isBlank()
        || body.contentBase64() == null
        || body.contentBase64().isBlank())
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Artifact content is not valid base64");
    }
    ...
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to store artifact");
    }
  }

  @PostMapping("/runs/{runKey}/browser/commands")
  public Map<String, Object> browserCommand(
      HttpServletRequest request, @PathVariable String runKey, @RequestBody BrowserCommand body) {
    ...
    long timeoutMs =
        body.timeoutMs() == null || body.timeoutMs() <= 0
            ? 30_000
  ...
  private long authenticate(HttpServletRequest request, String runKey) {
    ...
    String token =
        header != null && header.startsWith("Bearer ") ? header.substring(7).trim() : null;
    try {
