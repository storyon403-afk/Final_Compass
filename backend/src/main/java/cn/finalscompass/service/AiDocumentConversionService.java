...
public class AiDocumentConversionService {
  ...
  public AiDocumentConversionService(
      ...
      ObjectMapper json) {
    this.workerUri =
        workerUrl == null || workerUrl.isBlank()
            ? null
            : URI.create(workerUrl.replaceAll("/+$", "") + "/convert");
  ...
  public ConversionResult convert(MultipartFile file) {
    ...
    if (file.getSize() > MAX_BYTES)
      throw new ResponseStatusException(HttpStatus.PAYLOAD_TOO_LARGE, "单个附件不能超过 20MB");
    ...
    try {
      ...
      byte[] body =
          multipartBody(
              boundary, extension, safeContentType(file.getContentType()), file.getBytes());
      HttpRequest request =
          HttpRequest.newBuilder(workerUri)
  ...
  private byte[] multipartBody(String boundary, String extension, String contentType, byte[] data)
  ...
  public record ConversionResult(
      String fileName, String contentType, String markdown, int characters, boolean truncated) {}
