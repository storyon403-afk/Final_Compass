...
public class AiCredentialResolver {
    ...
    public ResolvedAiCredential resolve(long userId, String runtime, String providerValue, String modelValue,
        ...
        if (source == AiCredentialSource.PLATFORM) {
            ...
            SecretRow row = providerValue != null && !providerValue.isBlank()
            ...
            String selectedModel = modelValue == null || modelValue.isBlank() ? row.modelName()
                    : validateModel(row.provider(), modelValue);
            return new ResolvedAiCredential(row.provider(), selectedModel, source,
    ...
    private SecretRow platformProvider(String provider) {
        return jdbc.sql("""
                ...
                new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "管理员尚未启用该 Provider 通道"));
    }
    ...
    public ResolvedAiCredential resolvePlatformService(String providerValue,String modelValue) {
        String provider=providers.require(providerValue);SecretRow row=platformProvider(provider);
        return new ResolvedAiCredential(provider,validateModel(provider,modelValue),AiCredentialSource.PLATFORM,
                cipher.decrypt(row.encryptedKey(),row.encryptionIv()));
    }
    ...
    public ResolvedAiCredential resolvePlatformReview(long userId) {
        ...
        return new ResolvedAiCredential(row.provider(),row.modelName(),AiCredentialSource.PLATFORM,
                cipher.decrypt(row.encryptedKey(),row.encryptionIv()));
    }

    public ResolvedAiCredential resolveUserReview(long userId,String providerValue,String modelValue,
                                                   AiCredentialSource source,String ephemeral){
        ...
        if(source==AiCredentialSource.STORED_BYOK){UserSecretRow row=jdbc.sql("""
            SELECT provider,encrypted_key,encryption_iv FROM user_ai_review_secret
            WHERE user_id=:user AND provider=:provider
            """).param("user",userId).param("provider",provider).query(UserSecretRow.class).optional()
              .orElseThrow(()->new ResponseStatusException(HttpStatus.BAD_REQUEST,"尚未保存该 Provider 的 MultiWeb AI 审核 Key"));
            return new ResolvedAiCredential(provider,model,source,cipher.decrypt(row.encryptedKey(),row.encryptionIv()));}
        if(ephemeral==null||ephemeral.length()<8||ephemeral.length()>500)throw new IllegalArgumentException("请输入本次审核使用的 API Key");
        return new ResolvedAiCredential(provider,model,source,ephemeral.toCharArray());
    }
