...
public class ActivityService {
    ...
    public ActivityService(JdbcClient jdbc) { this.jdbc = jdbc; }
    ...
    public void award(long userId, String eventType, int points, String sourceType, String sourceRef) {
        jdbc.sql("""
            ...
            """).param("user", userId).param("event", eventType).param("points", points)
                .param("sourceType", sourceType).param("sourceRef", sourceRef).update();
    }
    ...
    public void ensureCurrentEntitlements() {
        ...
        Integer existing = jdbc.sql("SELECT COUNT(*) FROM ai_monthly_entitlement WHERE entitlement_month=:month")
                .param("month", entitlementMonth).query(Integer.class).single();
        ...
        jdbc.sql("""
            ...
            """).param("month", entitlementMonth).update();
    }

    public boolean hasPlatformEntitlement(long userId) {
        ensureCurrentEntitlements();
        return jdbc.sql("SELECT COUNT(*) FROM ai_monthly_entitlement WHERE entitlement_month=:month AND user_id=:user")
                .param("month", YearMonth.now().atDay(1)).param("user", userId).query(Integer.class).single() == 1;
    }
