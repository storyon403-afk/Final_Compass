...
public class AiAnalysisService {
    ...
    public Dashboard dashboard(long userId) {
        ...
        List<Map<String,Object>> reviewSecrets = jdbc.sql("""
            SELECT provider,key_fingerprint,key_label,consent_version,consented_at,updated_at
        ...
        Map<String,Object> reviewConfig = jdbc.sql(admin ? """
        ...
        List<Map<String, Object>> skills = admin ? jdbc.sql("""
        ...
        return new Dashboard(YearMonth.now().toString(), activity.currentMonthScore(userId),
                ...
                skills, providers.availableModelProviders(), providerConfigs, secrets, reviewSecrets,
                cipher.available(), admin ? configuredDefault : null,
                platformDefaultAvailable, hermesPlatformAvailable, reviewConfig);
    }
    ...
    public void deleteUserKey(long userId, String provider) {
        jdbc.sql("DELETE FROM user_ai_secret WHERE user_id=:user AND provider=:provider")
                .param("user", userId).param("provider", providers.require(provider)).update();
    }
    ...
    public Map<String,Object> saveUserReviewKey(long userId, SaveUserKey request) {
        ...
        if(!request.consentToStore())throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"保存 API Key 前必须获得用户明确同意");
        if(!cipher.available())throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE,"当前环境未配置 API Key 加密主密钥，只能使用不保存模式");
        char[] apiKey=requiredApiKey(request.apiKey());try{var encrypted=cipher.encrypt(apiKey);jdbc.sql("""
    ...
    public Map<String,Object> savePlatformDefault(long adminId, PlatformDefaultRequest request) {
        String provider = providers.require(request.provider());
        if (providers.available().stream().anyMatch(item -> item.id().equals(provider)
                && item.capabilities().contains("AGENT")))
    ...
    public Map<String,Object> savePlatformReviewKey(long adminId, SavePlatformKey request) {
        ...
        try {
            ...
            jdbc.sql("""
                ...
                """).param("provider",provider).param("model",cleanModel(request.model()))
                    .param("encrypted",encrypted.ciphertext()).param("iv",encrypted.iv())
                    .param("fingerprint",encrypted.fingerprint()).param("enabled",request.enabled())
    ...
    public record Dashboard(String month, int myScore, boolean platformEligible,
                            List<Map<String, Object>> leaderboard, List<Map<String, Object>> skills,
