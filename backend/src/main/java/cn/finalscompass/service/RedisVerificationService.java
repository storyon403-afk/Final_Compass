...
public class RedisVerificationService {
    ...
    public RedisVerificationService(StringRedisTemplate redis,
    ...
    public void checkSendLimits(String email, String ip) {
        ...
        consume(prefix + "email-daily:" + emailHash + ":" + LocalDate.now() + ":v1", 86400, 8, "该邮箱今日发送次数已达上限");
    ...
    public void store(long requestId, String email, String code) {
        requireConfigured();
        try {
            redis.opsForValue().set(codeKey(requestId), hmac(requestId, email, code), Duration.ofMinutes(10));
    ...
    public VerifyResult verify(long requestId, String email, String code) {
        ...
        try {
            ...
            if (MessageDigest.isEqual(expected.getBytes(StandardCharsets.UTF_8), hmac(requestId, email, code).getBytes(StandardCharsets.UTF_8))) {
                redis.delete(List.of(codeKey(requestId), failKey(requestId)));
                return VerifyResult.VALID;
            }
            Long failures = redis.opsForValue().increment(failKey(requestId));
            if (failures != null && failures == 1) redis.expire(failKey(requestId), Duration.ofMinutes(10));
            return failures != null && failures >= 5 ? VerifyResult.LOCKED : VerifyResult.INVALID;
        } catch (DataAccessException exception) { throw unavailable(); }
    }

    public void discard(long requestId) {
        try { redis.delete(List.of(codeKey(requestId), failKey(requestId))); }
        catch (DataAccessException ignored) { }
    }

    private void consume(String key, int ttl, int limit, String message) {
        try {
            Long result = redis.execute(LIMIT_SCRIPT, List.of(key), Integer.toString(ttl), Integer.toString(limit));
    ...
    private void requireConfigured() {
        if (pepper.length() < 16) throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "邮箱验证服务尚未配置");
    }
    ...
    private String hmac(long requestId, String email, String code) {
        try {
            ...
            return HexFormat.of().formatHex(mac.doFinal((requestId + ":" + email + ":" + code).getBytes(StandardCharsets.UTF_8)));
        } catch (Exception exception) { throw new IllegalStateException("验证码摘要生成失败", exception); }
    }
    ...
    private String codeKey(long id) { return prefix + "email-code:" + id + ":v1"; }
    private String failKey(long id) { return prefix + "verify-fail:" + id + ":v1"; }
    private ResponseStatusException unavailable() { return new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "邮箱验证服务暂不可用"); }
    public enum VerifyResult { VALID, INVALID, LOCKED, EXPIRED }
}
