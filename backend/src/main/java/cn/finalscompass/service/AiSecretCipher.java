...
public class AiSecretCipher {
    ...
    public String fingerprint(char[] secret) {
        ...
        } catch (Exception exception) { throw new IllegalStateException("无法生成 Key 指纹", exception); }
    }

    private SecretKeySpec key() {
        if (!available()) throw new IllegalStateException("服务器未配置 AI_SECRET_ENCRYPTION_KEY，不能保存 API Key");
        byte[] decoded;
        try { decoded = Base64.getDecoder().decode(configuredKey); }
        catch (IllegalArgumentException exception) { throw new IllegalStateException("AI_SECRET_ENCRYPTION_KEY 必须是 Base64", exception); }
        if (decoded.length != 32) throw new IllegalStateException("AI_SECRET_ENCRYPTION_KEY 解码后必须为 32 字节");
        return new SecretKeySpec(decoded, "AES");
    }

    private void requireSecret(char[] secret) {
        if (secret == null || secret.length < 8 || secret.length > 500) throw new IllegalArgumentException("API Key 长度不合法");
    }
