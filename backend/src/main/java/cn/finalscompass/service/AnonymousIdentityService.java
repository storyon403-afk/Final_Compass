...
public class AnonymousIdentityService {
  ...
  public AnonymousIdentityService(JdbcClient jdbc) {
    this.jdbc = jdbc;
  }
  ...
  public AnonymousProfile forAccount(long appUserId) {
    ...
    jdbc.sql(
            "INSERT IGNORE INTO anonymous_user(app_user_id, public_id, nickname) VALUES"
                + " (:accountId, :publicId, :nickname)")
        .param("accountId", appUserId)
        .param("publicId", publicId)
        .param("nickname", nickname)
        .update();
    return jdbc.sql("SELECT public_id, nickname FROM anonymous_user WHERE app_user_id = :accountId")
        .param("accountId", appUserId)
        .query(AnonymousProfile.class)
        .single();
  }
