...
public class MicrosoftGraphMailService {
    ...
    private static final String AUTHORIZE = "https://login.microsoftonline.com/common/oauth2/v2.0/authorize";
    ...
    public MicrosoftGraphMailService(JdbcClient jdbc, StringRedisTemplate redis, MailSecretCipher cipher, ObjectMapper json,
        ...
        this.jdbc = jdbc; this.redis = redis; this.cipher = cipher; this.json = json;
        this.clientId = clientId.trim(); this.clientSecret = clientSecret; this.redirectUri = redirectUri.trim();
    ...
    public String authorizationUrl(AuthService.CurrentUser admin) {
        ...
        redis.opsForValue().set(redisPrefix + state, admin.id() + ":" + verifier, Duration.ofMinutes(5));
        return AUTHORIZE + "?client_id=" + enc(clientId) + "&response_type=code&redirect_uri=" + enc(redirectUri)
                + "&response_mode=query&scope=" + enc(SCOPES) + "&state=" + enc(state)
                + "&code_challenge=" + enc(challenge) + "&code_challenge_method=S256&prompt=select_account";
    }
    ...
    public void complete(AuthService.CurrentUser admin, String code, String state) {
        ...
        if (stored == null || !stored.startsWith(admin.id() + ":")) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "微软授权状态已失效，请重新连接");
        Map<String, Object> token = token(Map.of("client_id", clientId, "client_secret", clientSecret,
                "grant_type", "authorization_code", "code", code, "redirect_uri", redirectUri,
                "code_verifier", stored.substring(stored.indexOf(':') + 1), "scope", SCOPES));
        String access = required(token, "access_token"), refresh = required(token, "refresh_token");
        Map<String, Object> me = graphGet(access, "https://graph.microsoft.com/v1.0/me?$select=displayName,mail,userPrincipalName");
        ...
        try {
            ...
            jdbc.sql("""
                    ...
                    """).param("email", email).param("name", String.valueOf(me.getOrDefault("displayName", "Finals Compass")))
                    .param("token", encrypted.ciphertext()).param("iv", encrypted.iv()).param("fingerprint", encrypted.fingerprint())
                    .param("scopes", String.valueOf(token.getOrDefault("scope", SCOPES))).param("admin", admin.id()).update();
        } finally { Arrays.fill(secret, '\0'); }
    }

    public void testAndEnable(AuthService.CurrentUser admin, String recipient) {
        send(recipient, "Finals Compass Microsoft Graph 测试", "Microsoft OAuth2 发信连接正常。\n\nFinals Compass");
        jdbc.sql("UPDATE mail_provider_setting SET active_provider='MICROSOFT_GRAPH',updated_by=:admin WHERE id=1")
    ...
    public void send(String recipient, String subject, String text) {
        Connection row = jdbc.sql("""
                ...
                """).query(Connection.class).optional()
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "微软邮箱尚未连接"));
        char[] refreshChars = cipher.decrypt(row.encryptedRefreshToken(), row.refreshTokenIv());
        try {
            Map<String, Object> token = token(Map.of("client_id", clientId, "client_secret", clientSecret,
                    "grant_type", "refresh_token", "refresh_token", new String(refreshChars), "scope", SCOPES));
            rotateRefreshToken(token);
            String body = json.writeValueAsString(Map.of("message", Map.of("subject", subject,
                    "body", Map.of("contentType", "Text", "content", text),
                    "toRecipients", List.of(Map.of("emailAddress", Map.of("address", recipient)))), "saveToSentItems", true));
            HttpRequest request = HttpRequest.newBuilder(URI.create("https://graph.microsoft.com/v1.0/me/sendMail"))
                    .timeout(Duration.ofSeconds(15)).header("Authorization", "Bearer " + required(token, "access_token"))
                    .header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(body)).build();
            HttpResponse<String> response = http.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() != 202) throw graphFailure("GRAPH_SEND_" + response.statusCode(), response.body());
            jdbc.sql("UPDATE mail_oauth_connection SET last_success_at=NOW(),last_error_code=NULL WHERE provider='MICROSOFT_GRAPH'").update();
        } catch (ResponseStatusException exception) { throw exception; }
        catch (Exception exception) { log.warn("Microsoft Graph mail failed: {}", exception.getClass().getSimpleName()); throw graphFailure("GRAPH_SEND_FAILED", null); }
        finally { Arrays.fill(refreshChars, '\0'); }
    }

    private void rotateRefreshToken(Map<String, Object> token) {
        Object rotated = token.get("refresh_token");
        if (rotated == null) { jdbc.sql("UPDATE mail_oauth_connection SET last_refreshed_at=NOW() WHERE provider='MICROSOFT_GRAPH'").update(); return; }
        ...
        try {
            ...
            jdbc.sql("""
                    ...
                    """).param("token", encrypted.ciphertext()).param("iv", encrypted.iv()).param("fingerprint", encrypted.fingerprint()).update();
        } finally { Arrays.fill(value, '\0'); }
    }

    private Map<String, Object> token(Map<String, String> form) {
        try {
            String body = form.entrySet().stream().map(e -> enc(e.getKey()) + "=" + enc(e.getValue())).collect(java.util.stream.Collectors.joining("&"));
            HttpRequest request = HttpRequest.newBuilder(URI.create(TOKEN)).timeout(Duration.ofSeconds(15))
            ...
            if (response.statusCode() / 100 != 2) throw graphFailure("OAUTH_TOKEN_" + response.statusCode(), String.valueOf(parsed.get("error")));
            return parsed;
        } catch (ResponseStatusException exception) { throw exception; }
        catch (Exception exception) { throw graphFailure("OAUTH_TOKEN_FAILED", null); }
    }

    private Map<String, Object> graphGet(String access, String uri) {
        try {
            HttpResponse<String> response = http.send(HttpRequest.newBuilder(URI.create(uri)).timeout(Duration.ofSeconds(10))
                    .header("Authorization", "Bearer " + access).GET().build(), HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() / 100 != 2) throw graphFailure("GRAPH_PROFILE_" + response.statusCode(), response.body());
            return json.readValue(response.body(), new TypeReference<>() {});
        } catch (ResponseStatusException exception) { throw exception; }
        catch (Exception exception) { throw graphFailure("GRAPH_PROFILE_FAILED", null); }
    }

    private ResponseStatusException graphFailure(String code, String detail) {
        try { jdbc.sql("UPDATE mail_oauth_connection SET last_error_code=:code WHERE provider='MICROSOFT_GRAPH'").param("code", code).update(); }
    ...
    private boolean configured() { return !clientId.isBlank() && !clientSecret.isBlank() && !redirectUri.isBlank() && cipher.available(); }
    private void requireConfigured() { if (!configured()) throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "服务器尚未配置 Microsoft OAuth2 应用"); }
    private String required(Map<String, Object> values, String key) { Object value=values.get(key); if(value==null||String.valueOf(value).isBlank()) throw graphFailure("MISSING_"+key.toUpperCase(),null); return String.valueOf(value); }
