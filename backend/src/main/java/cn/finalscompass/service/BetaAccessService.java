...
public class BetaAccessService {
  ...
  public BetaAccessService(
      JdbcClient jdbc,
      RedisVerificationService verification,
  ...
  public BetaAccessChallenge request(BetaAccessRequest request, String sourceIp) {
    ...
    jdbc.sql(
            "UPDATE beta_access_request SET status='EXPIRED' WHERE email=:email AND status IN"
                + " ('CREATED','CODE_SENT')")
        .param("email", email)
        .update();
    ...
    try {
      ...
      jdbc.sql(
              "UPDATE beta_access_request SET status='CODE_SENT',last_code_sent_at=NOW() WHERE"
                  + " id=:id")
          .param("id", id)
          .update();
      ...
      jdbc.sql("UPDATE beta_access_request SET status='EXPIRED' WHERE id=:id")
          .param("id", id)
          .update();
      throw exception;
    }
  ...
  public Map<String, String> verify(BetaAccessVerification request) {
    ...
    AccessRow row =
        jdbc.sql(
                "SELECT id,email,status,expires_at,failed_attempts FROM beta_access_request WHERE"
    ...
    if (!"CODE_SENT".equals(row.status()))
      throw new ResponseStatusException(HttpStatus.GONE, "该验证码已使用或已失效，请重新申请");
    ...
    switch (verification.verify(row.id(), email, request.code())) {
      case VALID -> {
        jdbc.sql(
                "UPDATE beta_access_request SET status='EMAIL_VERIFIED',verified_at=NOW() WHERE"
      ...
      case LOCKED -> {
        jdbc.sql(
                "UPDATE beta_access_request SET status='EXPIRED',failed_attempts=failed_attempts+1"
                    + " WHERE id=:id")
            .param("id", row.id())
            .update();
  ...
  private void reserveEventually(long requestId) {
    try {
      ...
      log.warn(
          "Verified beta request {} is awaiting account reservation: {}",
          requestId,
  ...
  private record AccessRow(
      long id, String email, String status, LocalDateTime expiresAt, int failedAttempts) {}
}
