...
public class MailSecretCipher {
    ...
    public boolean available() { return !configuredKey.isBlank(); }

    public EncryptedSecret encrypt(char[] secret) {
        if (secret == null || secret.length < 4 || secret.length > 8192) throw new IllegalArgumentException("邮件服务密钥长度不合法");
        ...
        try {
            ...
            return new EncryptedSecret(Base64.getEncoder().encodeToString(cipher.doFinal(plain)),
                    Base64.getEncoder().encodeToString(iv), fingerprint(secret));
        } catch (Exception exception) { throw new IllegalStateException("邮件服务密钥加密失败", exception); }
    ...
    public char[] decrypt(String encrypted, String iv) {
        ...
        } catch (Exception exception) { throw new IllegalStateException("邮件服务密钥解密失败，请检查邮件主密钥", exception); }
    }
    ...
    private SecretKeySpec key() {
        ...
        if (decoded.length != 32) throw new IllegalStateException("MAIL_SECRET_ENCRYPTION_KEY解码后必须为32字节");
        return new SecretKeySpec(decoded, "AES");
    }

    private String fingerprint(char[] value) throws Exception {
        byte[] bytes = new String(value).getBytes(StandardCharsets.UTF_8);
        try { return java.util.HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(bytes)).substring(0, 12); }
