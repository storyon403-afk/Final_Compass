...
public class AccountAllocationService {
    ...
    public String reserve(long requestId) {
        ...
        if (value == null) throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "内测账号分配失败");
        return value;
    }
    ...
    public void consumeOrRelease(long requestId, String actualUsername) {
        jdbc.sql("""
                ...
                """).param("username", actualUsername).param("request", requestId).update();
    }

    private String reserveInTransaction(long requestId) {
        String requestStatus = jdbc.sql("SELECT status FROM beta_access_request WHERE id=:id FOR UPDATE")
                .param("id", requestId).query(String.class).optional()
                .orElseThrow(() -> new IllegalArgumentException("申请不存在"));
        List<String> existing = jdbc.sql("SELECT reserved_username FROM account_reservation WHERE request_id=:request")
                .param("request", requestId).query(String.class).list();
        if (!existing.isEmpty()) return existing.getFirst();
        if (!"EMAIL_VERIFIED".equals(requestStatus)) throw new IllegalArgumentException("申请尚未完成邮箱验证");
        Sequence sequence = jdbc.sql("""
                ...
                """).query(Sequence.class).optional()
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "未配置内测账号序列"));
        long number = sequence.nextValue();
        for (int attempt=0;attempt<100;attempt++,number++) {
            String candidate = sequence.accountPrefix() + String.format("%0" + sequence.numberWidth() + "d", number);
            boolean occupied = jdbc.sql("""
                    ...
                    """).param("username",candidate).query(Integer.class).single()>0;
            jdbc.sql("UPDATE account_number_sequence SET next_value=:next WHERE id=:id")
                    .param("next",number+1).param("id",sequence.id()).update();
            ...
            jdbc.sql("""
                    ...
                    """).param("request",requestId).param("sequence",sequence.id()).param("username",candidate).param("value",number).update();
            return candidate;
        }
