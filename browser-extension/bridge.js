...
function emit(type, payload = {}) {
  window.postMessage({ source: SOURCE, type, extensionVersion: chrome.runtime.getManifest().version, ...payload }, '*')
}
emit('READY')
chrome.runtime.sendMessage({ type: 'REGISTER_PLATFORM_TAB' }).catch(() => {})
window.addEventListener('message', event => {
  if (event.source !== window || event.data?.source !== 'FINALS_COMPASS_WEBAPP') return
