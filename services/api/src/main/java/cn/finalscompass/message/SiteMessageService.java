...
public class SiteMessageService {
  private final JdbcClient jdbc; private final AuthorizationPolicy authorization;
  public SiteMessageService(JdbcClient jdbc,AuthorizationPolicy authorization){this.jdbc=jdbc;this.authorization=authorization;}
  ...
  @Transactional public void contactAdmin(AuthService.CurrentUser user,ContactInput input){int count=jdbc.sql("""
  ...
  @Transactional public Map<String,Integer> adminSend(AuthService.CurrentUser user,AdminSendInput input){authorization.requireAdmin(user);String target=input.targetUsername()==null?"":input.targetUsername().trim();int count;
    if(target.isEmpty())count=jdbc.sql("""
